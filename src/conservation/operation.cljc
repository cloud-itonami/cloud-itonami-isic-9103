(ns conservation.operation
  "OperationActor -- one zoo/botanical-garden/conservation operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (ConservationOps-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Conservation
  ...
  One graph run = one zoo/botanical-garden/conservation operation
  ...
  decision to a human operator (the conservationist/veterinarian). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:specimen/transfer`/`:specimen/release` ALWAYS reach
  this node when the governor is clean -- see `conservation.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [conservation.conservationopsllm :as conservationopsllm]
            [conservation.governor :as governor]
...
(defn build
  ...
  `conservation.store/Store`).
  opts:
    :advisor      -- a `conservation.conservationopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (conservationopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ConservationOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (conservationopsllm/-advise advisor store request)]
            {:proposal p :audit [(conservationopsllm/trace request p)]})))

      ;; Conservation Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
